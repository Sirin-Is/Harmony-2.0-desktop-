...
function employees() {
  ...
  const clients = getVisibleClients().filter((client) => (client.employees || []).length || (client.formerEmployees || []).length);
  const clientNames = [...new Set(clients.map((client) => client.name).filter(Boolean))].sort((left, right) => left.localeCompare(right, 'uk'));
  const filterActive = Array.isArray(uiState.hrEmployeeClientFilter);
  const selectedClients = new Set(filterActive ? uiState.hrEmployeeClientFilter : clientNames);
  const visibleClients = filterActive ? clients.filter((client) => selectedClients.has(client.name)) : clients;
  const rows = visibleClients.flatMap((client) => [...(client.employees || []), ...(client.formerEmployees || [])].map((employee) => {
  ...
  const clientHeading = `<span class="dashboard-header-label">ФОП<button type="button" class="table-filter-button${filterActive ? ' active' : ''}" data-hr-employee-filter aria-label="Фільтр за ФОП" aria-expanded="${uiState.hrEmployeeFilterOpen}">▾</button></span>`;
  const filterMenu = uiState.hrEmployeeFilterOpen ? `<div class="dashboard-filter-menu" data-hr-employee-filter-menu><div class="filter-menu-title">ФОП</div><label class="filter-select-all"><input type="checkbox" data-hr-employee-filter-select-all ${clientNames.every((name) => selectedClients.has(name)) ? 'checked' : ''}> Обрати все</label><div class="filter-options">${clientNames.map((name) => `<label><input type="checkbox" data-hr-employee-filter-option value="${esc(name)}" ${selectedClients.has(name) ? 'checked' : ''}> ${esc(name)}</label>`).join('')}</div><div class="filter-menu-actions"><button type="button" class="secondary" data-close-hr-employee-filter>Скасувати</button><button type="button" class="primary" data-apply-hr-employee-filter>Застосувати</button></div></div>` : '';
  return `<div class="toolbar section-control-row section-control-row-secondary"><button class="primary" data-add-employee>+ Працівник</button></div>${rows.length ? table(rows, [clientHeading, 'Працівник', 'Посада', 'Прийнято', 'Звільнено', 'Документи'], 'hr-table hr-employees-table') : empty('Найманих працівників поки немає.')}${filterMenu}`;
}
...
function orders(period) {
  ...
  return `<div class="toolbar section-control-row section-control-row-secondary"><button class="primary" data-add-hr-order>+ Документ</button></div>${rows.length ? table(rows, ['ФОП', '№', 'Дата', 'Суть документа', 'Працівник', 'Початок дії', 'Статус', ''], 'hr-table') : empty('Кадрових документів поки немає.')}`;
}
...
const input = (record, field) => {
  ...
  return field === 'paymentDate'
    ? `<input class="payroll-field payroll-date-field" type="date" data-payroll-id="${esc(record.id)}" data-payroll-field="${esc(field)}" value="${esc(record[field] || '')}" aria-label="${esc(label)}">`
...
function salary() {
  ...
  const clientPositions = new Map(clients.map((client, index) => [client.id, index]));
  ...
  const clientChronology = (recordsA, recordsB) => (clientPositions.get(recordsA[0].clientId) ?? Number.MAX_SAFE_INTEGER) - (clientPositions.get(recordsB[0].clientId) ?? Number.MAX_SAFE_INTEGER);
  ...
  const rows = [...byPaymentBatch.values()].sort((a, b) => groupChronology([a.type, a.records], [b.type, b.records])).flatMap(({ type, records: batchRecords }) => {
    ...
    const result = (label, rate, value) => gross ? `<span class="payroll-check ${Math.abs(check(rate, value)) < 1 ? 'ok' : 'warn'}" title="Перевірка ${label}"><b>${label}</b>${payrollMoney(check(rate, value))}</span>` : '';
    const esvRate = Number(employees.get(record.employeeId)?.esvRate || record.esvRate || 22) / 100;
    const moveControls = `<span class="payroll-client-order"><button type="button" class="payroll-drag-handle" data-payroll-client-drag="${esc(record.id)}" title="Перетягнути ФОП" aria-label="Перетягнути ФОП. Клавіші вгору та вниз також змінюють порядок">⠿</button></span>`;
    const statusCell = index === 0 ? `<td rowspan="${records.length}" class="payroll-status-cell"><select class="payroll-field payroll-status-field" data-payroll-id="${esc(record.id)}" data-payroll-field="status" aria-label="Статус виплати: ${esc(names.get(clientId) || record.clientName || 'ФОП')}"><option value="" ${record.status ? '' : 'selected'}></option>${['Набрано','Сплачено','Повідомлено','Сплачено невчасно'].map((s) => `<option ${record.status === s ? 'selected' : ''}>${s}</option>`).join('')}</select></td>` : '';
    const checks = gross ? `<td class="payroll-checks" aria-label="Перевірка податків"><div>${result('ПДФО', .18, record.pdfo)}${result('ВЗ', .05, record.vz)}${result('ЄСВ', esvRate, record.esv)}</div></td>` : '<td class="payroll-checks">-</td>';
    return `<tr${index === 0 ? ` data-payroll-client-row data-payroll-client-id="${esc(clientId)}"` : ''}>${index === 0 ? `<td rowspan="${records.length}" class="payroll-date">${moveControls}${input(record, 'paymentDate')}</td><td rowspan="${records.length}" class="payroll-client"><strong>${esc(shortName(names.get(clientId) || record.clientName || '-'))}</strong></td>` : ''}<td><button class="icon payroll-delete" data-delete-payroll="${esc(record.id)}" data-payroll-employee="${esc(record.employeeName || 'Працівник')}" data-payroll-type="${esc(type)}" data-payroll-amount="${esc(record.amount || '')}" title="Видалити рядок" aria-label="Видалити зарплатний рядок: ${esc(record.employeeName || 'працівник')}">✕</button>${esc(employeePayrollName(record.employeeName))}</td><td class="payroll-money-cell">${input(record, 'amount')}</td><td class="payroll-money-cell">${input(record, 'pdfo')}</td><td class="payroll-money-cell">${input(record, 'vz')}</td><td class="payroll-money-cell">${input(record, 'esv')}</td>${statusCell}<td class="payroll-gross-cell">${gross ? payrollMoney(gross) : '-'}</td>${checks}</tr>`;
    }))];
  });
  return `<div class="toolbar section-control-row section-control-row-secondary"><div class="toolbar-actions"><button class="secondary" data-payroll-prev ${month === 1 ? 'disabled' : ''} aria-label="Попередній місяць">←</button><strong class="calendar-period">${MONTH_NAMES_UA[month - 1]} ${settings.workingYear}</strong><button class="secondary" data-payroll-next ${month === 12 ? 'disabled' : ''} aria-label="Наступний місяць">→</button><button class="primary" data-add-payroll-client>+ Додати виплату</button><button class="secondary" data-add-payroll-employee>+ Працівник</button></div></div>${rows.length ? table(rows, ['Дата','ПІБ ФОП','ПІБ працівника','Сума виплати','ПДФО','ВЗ','ЄСВ','Статус','До оподаткування','Перевірка податків'], 'payroll-table') : empty('Додайте виплату — усі працівники вибраного ФОП з’являться окремими рядками.')}`;
}
...
export function renderHR() {
  ...
  return `<div class="subnav section-control-row section-control-row-primary" role="tablist"><button class="tab ${section === 'employees' ? 'active' : ''}" data-hr-section="employees">Наймані</button><button class="tab ${section === 'orders' ? 'active' : ''}" data-hr-section="orders">Документи по кадрам</button><button class="tab ${section === 'salary' ? 'active' : ''}" data-hr-section="salary">Виплата зарплати</button></div>${content}`;
}
