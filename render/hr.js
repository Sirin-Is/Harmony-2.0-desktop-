...
function documents() {
  ...
  const rows = clients.map((client) => {
    ...
    return `<tr><td><strong>${esc(shortName(client.name))}</strong></td><td>${statusButton(client.id, period, 'timesheetStatus', record.timesheetStatus, client.name, 'Табель робочого часу')}</td><td>${statusButton(client.id, period, 'payrollStatus', record.payrollStatus, client.name, 'Розрахунково-платіжна відомість')}</td><td>${statusButton(client.id, period, 'cashStatementStatus', record.cashStatementStatus, client.name, 'Відомість на виплату готівки')}</td></tr>`;
  });
...
function salary() {
  ...
  const paymentType = (type) => type || `Виплата зарплати за другу половину ${MONTH_NAMES_GENITIVE_UA[month === 1 ? 11 : month - 2]}`;
