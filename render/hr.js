...
const input = (record, field) => field === 'paymentDate'
  ? `<div class="payroll-date-control"><input class="payroll-field payroll-date-field" type="text" inputmode="numeric" maxlength="8" data-payroll-id="${record.id}" data-payroll-field="${field}" value="${esc(payrollDate(record[field]))}" placeholder="дд.мм.рр"><button type="button" class="payroll-date-picker" data-payroll-picker="${record.id}" title="Відкрити календар" aria-label="Відкрити календар">🗓</button><input class="payroll-native-date" type="date" tabindex="-1" data-payroll-id="${record.id}" data-payroll-native-date="${record.id}" value="${esc(record[field] || '')}"></div>`
  : `<input class="payroll-field" inputmode="decimal" data-payroll-id="${record.id}" data-payroll-field="${field}" value="${esc(record[field] || '')}" placeholder="-">`;
function salary() {
  const settings = getSettings();
