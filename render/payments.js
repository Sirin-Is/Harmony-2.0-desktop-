...
export function renderPayments() {
  ...
  const monthHeadRow = months.map((month) => `<th class="month-head">${month.label}<button type="button" class="auto-charge" data-autofill-month="${escapeHtml(month.key)}" title="Заповнити за вартістю обслуговування в картках ФОП">Авто</button></th>`).join('');
