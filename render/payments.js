...
export function renderPayments() {
  ...
  const monthHeadRow = months.map((month) => `<th class="month-head"><button type="button" class="month-autofill-label" data-autofill-month="${escapeHtml(month.key)}" title="Двічі натисніть, щоб заповнити суми за вартістю обслуговування" aria-label="Двічі натисніть, щоб автоматично заповнити ${month.label}">${month.label}</button></th>`).join('');
