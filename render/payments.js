...
export function renderPayments() {
  ...
  const monthHeadRow = months.map((month) => `<th class="month-head"><span class="month-head-content"><span>${month.label}</span><button type="button" class="auto-charge" data-autofill-month="${escapeHtml(month.key)}" title="Заповнити за вартістю обслуговування в картках ФОП" aria-label="Автоматично заповнити ${month.label}">✓</button></span></th>`).join('');
