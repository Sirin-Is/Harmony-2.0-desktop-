// render/inactive.js
// Builds the "Приховані ФОП" tab: clients hidden from Огляд/Оплати, with
...
export function renderInactive() {
  const rows = getArchivedClients().map((item) => `<tr>
    ...
    <td class="right row-actions">
      <button class="secondary" data-restore-client="${escapeHtml(item.id)}">Показати</button>
  ...
  return `<div class="toolbar"><p class="note">Приховані ФОП не беруть участі в обліку. Після підтвердження видалення клієнт одразу переходить у «Видалені», доступні через Налаштування.</p></div>
