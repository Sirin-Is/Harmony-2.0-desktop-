...
import { getAuditEvents } from '../state.js';
...
export function renderAudit() {
  ...
  return `<div class="toolbar audit-toolbar"><div class="audit-filters"><input id="auditSearch" placeholder="Пошук у журналі" value="${escapeHtml(uiState.auditSearch || '')}"><select id="auditSection">${options(unique(allEvents.map((item) => item.section)), uiState.auditSection, 'Усі розділи')}</select><select id="auditClient">${clientOptions}</select><select id="auditActor">${options(unique(allEvents.map((item) => item.actor)), uiState.auditActor, 'Усі користувачі')}</select><input id="auditDate" type="date" value="${escapeHtml(uiState.auditDate || '')}" title="Дата"><select id="auditStatus"><option value="">Усі статуси</option><option value="active" ${uiState.auditStatus === 'active' ? 'selected' : ''}>Активні</option><option value="cancelled" ${uiState.auditStatus === 'cancelled' ? 'selected' : ''}>Скасовані</option></select><button class="secondary" data-audit-rollback>↶ Відкат змін</button></div></div>${table(rows, ['Дата', 'Час', 'Користувач', 'Розділ', 'ФОП', 'Поле', 'Попереднє значення', 'Нове значення', 'Статус'], 'audit-table')}`;
}
