...
export function getDefaultReportDeadline(db: Database, realGroup: string, periodKey: string): string {
  if (realGroup === '3') {
    const legacy = periodKey.startsWith(`${DEFAULT_WORKING_YEAR}-`) ? db.settings.reportDeadlines.quarterly[periodKey.slice(5)] : '';
    return db.settings.reportDeadlines.quarterly[periodKey] || legacy || '';
  }
  return db.settings.reportDeadlines.annual[periodKey] || '';
}
